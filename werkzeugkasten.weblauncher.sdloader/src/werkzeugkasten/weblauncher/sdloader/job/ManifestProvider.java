...
public class ManifestProvider implements IManifestProvider {
	 ...
	 * 
	 * @param manifest
	 *            the manifest to which the entries should be added
	 * @param jarPackage
	 *            the JAR package specification
	 */
		...
		if (jarPackage.isJarSealed()) {
			...
			if (packages != null) {
				for (int i = 0; i < packages.length; i++) {
					...
					manifest.getEntries().put(getInManifestFormat(packages[i]),
							attributes);
				}
			}
		} else {
	...
	private void putMainClass(Manifest manifest, JarPackageData jarPackage) {
		if (jarPackage.getManifestMainClass() != null
				&& jarPackage.getManifestMainClass().getFullyQualifiedName()
						.length() > 0)
