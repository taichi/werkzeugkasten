...
public class WhitespaceCharacterPainter implements IPainter, PaintListener {
	...
	private void handleDrawRequest(GC gc, int x, int y, int w, int h) {
		...
		int startLine = (y + fTextWidget.getTopPixel())
				/ fTextWidget.getLineHeight();
		int endLine = (y + h - 1 + fTextWidget.getTopPixel())
				/ fTextWidget.getLineHeight();
		if (startLine <= endLine && startLine < lineCount) {
			int startOffset = fTextWidget.getOffsetAtLine(startLine);
			int endOffset = endLine < lineCount - 1 ? fTextWidget
					.getOffsetAtLine(endLine + 1) : fTextWidget.getCharCount();
	...
	private void redrawAll(boolean redrawBackground) {
		...
		int startOffset = fTextWidget.getOffsetAtLine(startLine);
		int endLine = 1
				+ (fTextWidget.getTopPixel() + fTextWidget.getClientArea().height)
				/ fTextWidget.getLineHeight();
		int endOffset;
